...
void testInsertIntValuesWithMaxElementsPerNode_3(const std::string &indexName) {
    ...
    int indexIsUnique = 1;
    DBMyIndex2 &index = *(DBMyIndex2 *) getClassForName(indexName, 5, &bufMgr, &file1, AttrTypeEnum::INT, ModType::WRITE,
                                                      indexIsUnique);
    index.initializeIndex();
    LOG4CXX_INFO(logger, "Begin testing...");
    //TEST-1----------------------------------------------------------------
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        testIndexInitialization("DBMyIndex2");
        testInsertIntValuesWithMaxElementsPerNode_3("DBMyIndex2");
    } catch (Exception &) {
        rc = EXIT_FAILURE;
    }
